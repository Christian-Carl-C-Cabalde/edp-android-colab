...
class UserRepository(
    ...
    suspend fun login(email: String, password: String): AppResult<User> = safeCall {
        // TODO 6a: search the server for the (trimmed) email
        val matches = findUsers(email.trim())

        // TODO 6b: the FIRST user whose email equals the typed email
        // (case-insensitive) AND whose password equals the typed password
        val found = matches.firstOrNull {
            it.email.equals(email.trim(), ignoreCase = true) && it.password == password
        }

        // TODO 6c: the last line of the block is the result
    ...
    suspend fun register(
        ...
        // TODO 7a: true when ANY user from the search has exactly this email
        val taken = findUsers(email.trim())
            .any { it.email.equals(email.trim(), ignoreCase = true) }

        // TODO 7b + 7c: the last expression is the result
        if (taken) {
